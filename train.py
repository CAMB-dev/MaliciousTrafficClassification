...
train_dataset = TrafficDataset(train_data, tokenizer, max_length=MAX_LENGTH, label_dict=LABEL_DICT)
...
def train_epoch(model, data_loader, optimizer, device, epoch):
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        # 日志记录
        logger.info(f"Epoch [{epoch+1}/{EPOCHS}], Step [{batch_idx+1}/{len(data_loader)}], Loss: {loss.mean().item():.4f}")
...
for epoch in range(EPOCHS):
    ...
    logger.info(f"Epoch [{epoch + 1}/{EPOCHS}] completed with Average Loss: {train_loss:.4f}")
